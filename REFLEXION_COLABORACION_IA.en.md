...
this way — a person with lived knowledge of a specific game (he
played it as a kid, on an actual MSX, in 1987/88) and an AI
assistant with no memory of its own between sessions, relying on a
text file (`FINDINGS.md`) as a bridge. It's not a piece of
marketing about how well AI works, nor a complaint about its limits
— it's an honest attempt to describe the real dynamic, with its
successes and its friction, using this specific project as a case
study.
...
worked because of having "a very powerful AI" nor "a very
expert user" — it has worked because of having a method that didn't
depend on either side always being right. Byte-for-byte
